...
if(number1>number2):
    print(words[number1:number2:-1])
else:
    print("First number must be greater then second number!")"""
...
for x in range(8):
 ...
 if (userNumber >= 5 and userNumber <= 7):
     for x in range(userNumber):
 ...
 elif userNumber<5 and userNumber>=3:
     for x in range(userNumber):
        tmp=random.choice(words)
        pw=pw+tmp
        rePw=pw[::-1]
...
for x in united_we_stand:
    print(x,united_we_stand[x])


del united_we_stand["Nat"]
print(united_we_stand)

sort=sorted(united_we_stand.items())
print(sort)
